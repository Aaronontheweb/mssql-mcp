...
public class SqlExecutionToolTests : IAsyncLifetime
{
    ...
    #region DescribeTable Tests

    [Fact]
    public async Task DescribeTable_Users_ReturnsTableStructure()
    {
        // Act
        var result = await _tool.DescribeTable("Users");
        ...
        Assert.Contains("nvarchar", result);
        Assert.Contains("datetime2", result);
        Assert.Contains("bit", result);
        
        // Should indicate primary key
        Assert.Contains("YES", result); // Primary key indicator
    }

    [Fact]
    public async Task DescribeTable_WithSchema_ReturnsTableStructure()
    {
        // Act
        var result = await _tool.DescribeTable("TestSchema.Products");
    ...
    [Fact]
    public async Task DescribeTable_OrdersWithForeignKey_ShowsReferences()
    {
        // Act
        var result = await _tool.DescribeTable("Orders");

        // Assert
        Assert.NotNull(result);
        
        // Should contain foreign key information
        Assert.Contains("UserId", result);
        Assert.Contains("dbo.Users", result); // Should show the referenced table
    }

    [Fact]
    public async Task DescribeTable_NonExistentTable_ReturnsNotFound()
    {
        // Act
        var result = await _tool.DescribeTable("NonExistentTable");
    ...
    [Fact]
    public async Task DescribeTable_EmptyTableName_ReturnsError()
    {
        // Act
        var result = await _tool.DescribeTable("");

        // Assert
        Assert.NotNull(result);
        Assert.Contains("Error: Table name cannot be empty", result);
    }
